...
# And hard-code classical 2nd order fvm  
@kernel function vert_fvm_interface_tendency!(
    ...
    @inbounds begin
        ...
        if periodicstack
            ...
            @unroll for s in 1:num_state_prognostic
                tendency[n, s, e] -= α * vMI * sM_bottom * local_flux_bottom[s]
            end
    ...
    @inbounds for eV in 2:(nvertelem - 1)
        ...
        # volume mass inverse
        vMI = vgeo[_MI, n, e]
        cw⁻, cw, cw⁺ = vgeo[_M⁻, n, e], vgeo[_M, n, e], vgeo[_M⁺, n, e]
        ...
        # compute the flux
        fill!(local_flux_bottom, -zero(eltype(local_flux_top)))
        ...
        @unroll for s in 1:num_state_prognostic
            tendency[n, s, e⁻] += α * vMI * sM_bottom * flux_bottom[s]
        end
        @unroll for s in 1:num_state_prognostic
            tendency[n, s, e] -= α * vMI * sM_bottom * flux_bottom[s]
        end
    ...
    @inbounds begin
        ...
        if periodicstack
            ...
            @unroll for s in 1:num_state_prognostic
                tendency[n, s, e] -= α * vMI * sM_top * flux_top[s]
            end
            @unroll for s in 1:num_state_prognostic
                tendency[n, s, e⁺] -= α * vMI * sM_top * flux_top[s]
            end
            ...
            @unroll for s in 1:num_state_prognostic
                tendency[n, s, e] -= α * vMI * sM_top * local_flux_top[s]
            end
