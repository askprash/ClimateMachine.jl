...
"""
struct RainSnow_1M{
    PV <: Union{Mass, Energy, TotalMoisture, LiquidMoisture, IceMoisture, Rain, Snow},
} <: TendencyDef{Source, PV} end
...
function compute_rain_snow_params(atmos, state, aux, t, ts)
    ...
    tmp = conv_q_ice_to_q_sno(atmos.param_set, atmos.param_set.microphys.ice, q, state.ρ, T)
    S_qs += tmp
    S_qi -= tmp
    S_ρe -= state.ρ * tmp * (I_i + Φ)

    # sink of cloud water via accretion cloud water - rain
    tmp = accretion(
        atmos.param_set,
        atmos.param_set.microphys.liq,
        atmos.param_set.microphys.rai,
    ...
    # sink of cloud ice via accretion cloud ice - snow
    tmp = accretion(
        atmos.param_set,
    ...
    S_qr -= (tmp2 - tmp1)
    S_qs += tmp2
    S_ρe -= state.ρ * tmp1 * (I_i + Φ) - state.ρ * _L_f * tmp2
    ...
    return (S_qt = S_qt, S_ql = S_ql, S_qi = S_qi, S_qr = S_qr, S_qs = S_qs, S_ρe = S_ρe)
end
